...
// 与 Java 分派名对齐，可随协议扩展
export type BridgeMethod = 'listNotes' | 'getNoteById' | 'saveNote' | 'deleteNote'

// 各 method 的入参（由 invoke 在发往原生前整体 JSON.stringify，并附带 callbackId）
export type BridgeParams = {
  listNotes: { offset?: number; limit?: number }
  getNoteById: { id: number }
  saveNote: { id?: number; title?: string; content_md?: string; contentMd?: string }
...
export type Invoke = <M extends BridgeMethod>(method: M, params: BridgeParams[M]) => Promise<BridgeData[M]>
...
function runMock(method: BridgeMethod, body: Record<string, unknown>, onResult: (p: NativePayload) => void) {
  const callbackId = String(body.callbackId ?? '')
  const ok = (data: unknown) => onResult({ callbackId, ok: true, data })
  const err = (code: string, message: string) => onResult({ callbackId, ok: false, code, error: message })
  const now = Date.now()
  if (method === 'listNotes') {
    ok({
      notes: [
        { id: 1, title: 'Mock 示例 A', contentMd: '# 标题\n\n在浏览器 mock 中查看。', updatedAt: now, isDeleted: 0 },
        { id: 2, title: 'Mock 示例 B', contentMd: '正文 **粗体**', updatedAt: now - 1000, isDeleted: 0 },
      ],
    })
    return
  }
  if (method === 'getNoteById') {
    ...
    ok({ note: { id, title: 'Mock 笔记 ' + id, contentMd: '# 笔记 ' + id, updatedAt: now, isDeleted: 0 } })
    return
  }
  if (method === 'saveNote') {
